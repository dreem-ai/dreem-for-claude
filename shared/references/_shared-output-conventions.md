...
- **Always also print the raw output URL as plain unformatted text right after**, labeled as the full-resolution original (e.g. "Full res: `https://...`") - not inside `[...]()`. The inline chat render can be scaled down for display even when the underlying asset is 2K/4K; the raw URL is how the user opens or downloads the true output in a browser tab. Do this every time, not only when the host fails to render MCP Apps - and say plainly it must be copied/opened whole, not clicked from a shortened or reformatted version.
